...
const runRequest = async (
    ...
    service: (requestValidationOutput: RequestValidationOutput) => Promise<unknown>,
) => {
    const {request, response,} = call;

    try {
        const requestValidationOutput: RequestValidationOutput = await validateRequest(request, security);
